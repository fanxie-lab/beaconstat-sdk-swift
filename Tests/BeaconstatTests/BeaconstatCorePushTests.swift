...
final class BeaconstatCorePushTests: XCTestCase {
    ...
    func testPushReceived() {
        let c = makeCore()
        c.trackPushReceived(category: "MESSAGE_CATEGORY", wasSilent: true); c.flush()
        let done = expectation(description: "flow"); c.onQuiescent { done.fulfill() }
        wait(for: [done], timeout: 3)
        let body = sentBodies()
        XCTAssertTrue(body.contains("_bcs.apple.push_received"))
        XCTAssertTrue(body.contains("_bcs.apple.push_category"))
        XCTAssertTrue(body.contains("MESSAGE_CATEGORY"))
        XCTAssertTrue(body.contains("_bcs.apple.push_was_silent"))
        XCTAssertTrue(body.contains("_bcs.session.id"))
    }

    func testPushOpened() {
        let c = makeCore()
        c.trackPushOpened(category: "MESSAGE_CATEGORY", actionId: "REPLY_ACTION"); c.flush()
        let done = expectation(description: "flow"); c.onQuiescent { done.fulfill() }
        wait(for: [done], timeout: 3)
        let body = sentBodies()
        XCTAssertTrue(body.contains("_bcs.apple.push_opened"))
        XCTAssertTrue(body.contains("_bcs.apple.push_action_id"))
        XCTAssertTrue(body.contains("REPLY_ACTION"))
    }

    func testPushReceivedNilCategoryOmitsKey() {
        let c = makeCore()
        c.trackPushReceived(category: nil, wasSilent: false); c.flush()
        let done = expectation(description: "flow"); c.onQuiescent { done.fulfill() }
        wait(for: [done], timeout: 3)
        let body = sentBodies()
        XCTAssertTrue(body.contains("_bcs.apple.push_received"))
        XCTAssertFalse(body.contains("_bcs.apple.push_category")) // omitted when nil
        XCTAssertTrue(body.contains("\"_bcs.apple.push_was_silent\":\"false\""))
    }
}
